...
async function showChatRollMessage(r, zeromode, attribute_name = "", position = "", effect = "", note = "", current_stress, current_crew_tier) {
  ...
  // Check for Indugle Vice roll
    ...
    result = await renderTemplate("systems/runners-in-the-shadows/templates/chat/vice-roll.html", {rolls: rolls, roll_status: roll_status, attribute_label: attribute_label, clear_stress: clear_stress, note: note});
  }
  // Check for Gather Information roll
  else if (attribute_name == 'BITD.GatherInformation') {
    result = await renderTemplate("systems/runners-in-the-shadows/templates/chat/gather-info-roll.html", {rolls: rolls, roll_status: roll_status, attribute_label: attribute_label, note: note});
  }
  // Check for Engagement roll
  else if (attribute_name == 'BITD.Engagement') {
    result = await renderTemplate("systems/runners-in-the-shadows/templates/chat/engagement-roll.html", {rolls: rolls, roll_status: roll_status, attribute_label: attribute_label, note: note});
  }
  // Check for Asset roll
    ...
    result = await renderTemplate("systems/runners-in-the-shadows/templates/chat/asset-roll.html", {rolls: rolls, roll_status: roll_status, attribute_label: attribute_label, tier_quality: tier_quality, note: note});
  }
  // Fortune roll if not specified
  else {
    result = await renderTemplate("systems/runners-in-the-shadows/templates/chat/fortune-roll.html", {rolls: rolls, roll_status: roll_status, attribute_label: "BITD.Fortune", note: note});
  }
